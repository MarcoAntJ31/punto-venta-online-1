...
export type AButtonColors = 'base' | 'blue' | 'purple' | 'green' | 'red' | 'yellow'
export type AButtonSizes = 'x-small' | 'small' | 'default' | 'large' | 'x-large'
export type AButtonVariants = 'text' | 'flat' | 'elevated' | 'tonal' | 'outlined' | 'plain' | 'radio'
  ...
  return theme.global.current.value.dark || preferredColor.value === 'light';
});
  ...
  switch (props.color) {
    case 'base':
      if (props.variant === 'outlined' || props.variant === 'text') {
        return inDarkMode.value ? '#6B6C6D' : '#6B6C6D'; // Colores para variantes en modo claro
      }
      return inDarkMode.value ? '#202121' : '#F7F9FA';
    case 'blue':
      return inDarkMode.value ? '#212F49' : '#85C1E9'; // Azul en modo claro
    case 'purple':
    ...
    default:
      return inDarkMode.value ? '#202121' : '#F7F9FA'; // Valor por defecto en modo claro
  }
});
  </script>